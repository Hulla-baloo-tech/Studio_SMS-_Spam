...
def main():
    ...
    print("[3] ออก.")
    print()
    number = input(f"{BRIGHT}{BLUE}ใส่จำนวนที่ต้องการ: {RESET_ALL}")
    if number == "1":
        spam_handler()
    elif number == "2":
        update()
        ...
        print(f"\n{BRIGHT}{RED}[*] ป้อนหมายเลขรายการไม่ถูกต้อง!{RESET_ALL}")
        sleep(1)
        main()
...
def spam_handler():
    ...
    try:
        ...
        threads = "1"
    banner()
    print(f"Telephone: {BRIGHT}{BLUE}{phone}{RESET_ALL}")
    print("Spammer launched", end="\n")
    print(f"{BRIGHT}{RED}[*] Ctrl+Z to stop{RESET_ALL}")
...
def check_version():
    ...
    if float(version) < float(get("https://raw.githubusercontent.com/cludeex/spammer/master/version.txt").text):
        print(f"\n{BRIGHT}{RED}[*] The version is outdated and needs to be updated.!{RESET_ALL}")
        sleep(1)
        main()
    return
